...
class Game(object):
    ...
    def printIntoLogs(self, agentIndex, agent, action, new_state):
        """
        Saves the relevant information from a state in the file logs.txt.
        
        *parameters:
        -agentIndex: index of the agent
        -agent: agent that contains the method printLineData
        -action: action that the agent takes in the current tick
        -new_state: sucessor state to get the new score
        """
        if agentIndex == 0:
            file = open(new_state.logs_file,"a")
            log = agent.printLineData(self.state) + action + ", " + str(new_state.getScore()) + "\n"
            file.write(log)
    ...
    def run( self ):
        ...
        while not self.gameOver:
            ...
            if self.catchExceptions:
                try:
                    new_state = self.state.generateSuccessor( agentIndex, action )
                except Exception as data:
                    self.mute(agentIndex)
                    self._agentCrash(agentIndex)
                    self.unmute()
                ...
                new_state = self.state.generateSuccessor( agentIndex, action )
            #Printing into logs.txt the relevant information about the state    
            # self.printIntoLogs(agentIndex,agent,action,new_state)
